...
import { Send, StopCircle, Sparkles, ShieldCheck, Loader2, ChevronLeft, Check, Zap } from 'lucide-react'
import { useCouncilV2Stream } from '@/hooks/useCouncilV2Stream'
import { useCouncilV2Store } from '@/store/councilV2Store'
import { SEVEN_AGENTS } from '@/types/council'
  ...
  return (
    <button
      ...
      style={{
        ...
        borderLeftWidth: isOpen ? '3px' : '3px',
    ...
    >
      ...
      <span
        className="flex-1 text-[14px] font-semibold font-heading truncate transition-colors duration-200"
        ...
        >
          {r.n < 3 ? (
            <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold ${
              currentRound > r.n ? 'bg-emerald-100 text-emerald-700' : currentRound === r.n ? 'bg-blue-100 text-blue-700' : 'bg-gray-200 text-gray-400'
            }`}>
              {currentRound > r.n ? '✓' : r.n}
            </span>
          ) : (
    ...
    <div className="space-y-6 animate-in-up p-8 rounded-[2.5rem] relative overflow-hidden backdrop-blur-3xl"
          ...
          return (
            <div key={agent.key} className="flex items-center gap-4">
              ...
              <div className="flex-1 h-2.5 bg-gray-200/50 rounded-full overflow-hidden shadow-inner">
                <div
                  className="h-full rounded-full transition-all duration-700 shadow-[0_0_10px_rgba(0,0,0,0.1)]"
      ...
      {result.summary && (
        <div className="relative z-10 mt-6 p-6 bg-white/50 rounded-3xl border border-white/60 backdrop-blur-md shadow-inner">
          <div className="prose prose-violet max-w-none text-gray-800 font-inter font-medium leading-relaxed
                          prose-headings:font-outfit prose-headings:font-black prose-strong:font-bold">
  ...
  return (
    <div className="space-y-6 animate-in-up p-8 rounded-[2.5rem] relative overflow-hidden backdrop-blur-3xl"
      ...
      <div className="relative z-10 p-8 bg-white/40 rounded-3xl border border-white/60 backdrop-blur-md shadow-inner">
        <div className="prose prose-emerald max-w-none text-gray-800 font-inter text-[16px] leading-relaxed
                        prose-headings:font-outfit prose-headings:font-black prose-strong:font-bold prose-strong:text-emerald-900">
...
export default function Chat() {
  ...
  const { startStream, stopStream, isStreaming } = useCouncilV2Stream()
  ...
  const selectedAgentInfo = SEVEN_AGENTS.find((a) => a.key === selectedAgent)
          ...
          >
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-5 mb-10">
                <div 
                  className="w-5 h-5 rounded-full shrink-0 shadow-[0_0_20px] animate-pulse relative" 
                  style={{ 
                    backgroundColor: selectedAgentInfo.hexColor,
                    boxShadow: `0 0 20px ${selectedAgentInfo.hexColor}80`
                  }} 
                >
              ...
              <div className="text-gray-700 font-inter leading-relaxed text-[17px] font-medium
                ...
                <MarkdownRenderer content={roundState.output || ''} />
              </div>
            </div>
          </div>
        </div>
            ...
            {hasStarted ? getMainContent() : (
              <div className="flex flex-col items-center justify-center h-full text-gray-400">
                ...
                <p className="text-[16px] text-slate-500 font-inter mt-3 max-w-md text-center leading-relaxed">
                  Ask a question to activate 7 AI agents who will automatically analyze, debate, and deliver a final verdict through 3 rigorous rounds.
                </p>
            ...
            {SEVEN_AGENTS.map((agent) => {
              ...
              return (
                <AgentTab
                  ...
                  agent={agent}
                  isOpen={isOpen}
                  onClick={() => handleTabClick(agent.key)}
            ...
            <div className="flex overflow-x-auto gap-2 p-2">
              ...
              {(moderatorR1 || (isStreaming && currentRound >= 1)) && (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold whitespace-nowrap border transition-all duration-200 shrink-0 ${
                    viewMode === 'moderator' ? 'bg-violet-50 border-violet-300 text-violet-700' : 'bg-white border-gray-200 text-gray-700'
                  }`}
                >
              ...
              {(supervisorResult || (isStreaming && currentRound >= 3)) && (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold whitespace-nowrap border transition-all duration-200 shrink-0 ${
                    viewMode === 'supervisor' ? 'bg-emerald-50 border-emerald-300 text-emerald-700' : 'bg-white border-gray-200 text-gray-700'
                  }`}
                >
